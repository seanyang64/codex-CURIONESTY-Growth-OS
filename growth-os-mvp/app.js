(function () {
  ...
  const enrollmentStatuses = ["报名", "进行中", "已完成", "退出", "旁听"];
...
b. 突出表现奖学金：在自己感兴趣领域持续付出精力、时间与努力并产出优异成果（能够被好奇当作有效的成长案例对外讲述的）。`;
  ...
  const studentExportFields = [
    ...
    { key: "courseTotal", label: "修课记录数" },
    { key: "courseTypes", label: "课程类型统计" }
  ];
  ...
  let courseCalendarCursor = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
  let selectedCourseCalendarEvent = null;
  ...
  function reportRangeText(range = {}) {
  ...
  function motivationReportVisible(record) {
    return record.reportVisible !== false;
  }
  ...
  function calculateGpaFromGrades(grades) {
  ...
  function calculateGpa(studentId) {
    ...
    return calculateGpaFromGrades(grades);
  }

  function enrollmentKey(studentId, courseId, term = "") {
    return `${studentId || ""}|${courseId || ""}|${term || ""}`;
  }

  function courseTypeLabels(course) {
    const attributes = courseAttributes(course);
    if (attributes.length) return attributes;
    const paths = coursePaths(course);
    if (paths.length) return paths.map((path) => `${path}路径`);
    return [course?.center || "未分类课程"];
  }

  function findGradeForCourse(studentId, courseId, term = "") {
    const grades = state.grades
      .filter((grade) => grade.studentId === studentId && grade.courseId === courseId)
      .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
    if (!grades.length) return null;
    return grades.find((grade) => (grade.term || "") === (term || "")) || grades[0];
  }

  function studentCourseRecords(studentId, range = {}) {
    const records = new Map();
    state.enrollments
      .filter((record) => record.studentId === studentId)
      .filter((record) => dateInRange(getCourse(record.courseId)?.startAt || record.createdAt, range))
      .forEach((record) => {
        records.set(enrollmentKey(record.studentId, record.courseId, record.term), {
          ...record,
          source: "enrollment"
        });
      });
    ...
    state.grades
      ...
      .filter((grade) => dateInRange(grade.createdAt, range))
      .forEach((grade) => {
        const key = enrollmentKey(grade.studentId, grade.courseId, grade.term);
        if (!records.has(key)) {
          records.set(key, {
            id: `grade_link_${grade.id}`,
    ...
    return [...records.values()]
      .map((record) => {
        ...
        const grade = findGradeForCourse(record.studentId, record.courseId, record.term);
        return {
          ...record,
          course,
          grade,
          types: courseTypeLabels(course)
        };
      })
      .sort((a, b) => {
        const aDate = a.course?.startAt || a.createdAt || "";
        const bDate = b.course?.startAt || b.createdAt || "";
        return String(bDate).localeCompare(String(aDate));
      });
  }

  function studentCourseSummary(studentId, range = {}) {
    const records = studentCourseRecords(studentId, range);
    const typeMap = new Map();
    records.forEach((record) => {
      record.types.forEach((type) => {
        if (!typeMap.has(type)) {
          typeMap.set(type, { type, total: 0, completed: 0, graded: 0, scoreSum: 0 });
        }
        const row = typeMap.get(type);
        row.total += 1;
        if (["已完成"].includes(record.status) || record.grade) row.completed += 1;
        if (record.grade) {
          row.graded += 1;
          row.scoreSum += Number(record.grade.score || 0);
        }
      });
    });
    const byType = [...typeMap.values()].map((row) => ({
      ...row,
      average: row.graded ? Math.round((row.scoreSum / row.graded) * 10) / 10 : null
    }));
    return {
      records,
      total: records.length,
      completed: records.filter((record) => record.status === "已完成" || record.grade).length,
      graded: records.filter((record) => record.grade).length,
      byType
    };
  }

  function upsertEnrollmentFromGrade(studentId, courseId, term) {
    if (!studentId || !courseId) return;
    const normalizedTerm = term || "";
    const existing = state.enrollments.find((record) => {
      return record.studentId === studentId
        && record.courseId === courseId
        && (record.term || "") === normalizedTerm;
    });
    if (existing) {
      if (existing.status !== "旁听") existing.status = "已完成";
      return;
    }
  ...
  function reportMotivationCandidates() {
    const studentId = $("#reportStudent")?.value;
    const range = reportDateRange();
    if (!studentId) return [];
    return state.motivation
      .filter((record) => record.studentId === studentId)
      .filter((record) => dateInRange(record.createdAt, range))
      .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
  }

  function renderReportMotivationPicker() {
    const container = $("#reportMotivationPicker");
    if (!container) return;
    const records = reportMotivationCandidates();
    container.innerHTML = records.map((record) => `
      <label class="report-record-row ${motivationReportVisible(record) ? "" : "internal-only"}">
        <input type="checkbox" name="reportMotivationIds" value="${escapeHtml(record.id)}" ${motivationReportVisible(record) ? "checked" : ""}>
        <span>
          <strong>${escapeHtml(formatDate(record.createdAt))} · ${escapeHtml((record.dimensions || []).join(" / ") || "未设维度")}</strong>
          <small>${escapeHtml(record.scene || "未设场景")} · ${escapeHtml(record.statusTone || "")} · ${motivationReportVisible(record) ? "报告素材" : "仅内部"}</small>
          <em>${escapeHtml(record.fact || "暂无事实描述")}</em>
        </span>
      </label>
    `).join("");
    emptyIfNeeded(container);
  }
  ...
  function renderCourseEnrollmentStudentSelect() {
    const select = $("#courseEnrollmentStudent");
    if (!select) return;
    const current = select.value;
    const students = filteredSortedStudents(selectedCourseStatuses, selectedCourseAcademies);
    select.innerHTML = students.length
      ? students.map((student) => `<option value="${student.id}">${escapeHtml(student.name)} · ${escapeHtml(student.studentNo)}</option>`).join("")
      : `<option value="">没有符合筛选的学生</option>`;
    if (current && students.some((student) => student.id === current)) select.value = current;
  ...
  function renderCourseEnrollmentStats() {
    const studentSelect = $("#courseEnrollmentStudent");
    const stats = $("#studentCourseStats");
    const list = $("#courseEnrollmentList");
    if (!studentSelect || !stats || !list) return;
    const studentId = studentSelect.value;
    const summary = studentCourseSummary(studentId);
    stats.innerHTML = studentId ? `
      <div class="stats-grid compact-stats course-summary-grid">
        <div class="stat-block"><span>修课记录</span><strong>${summary.total}</strong></div>
        <div class="stat-block"><span>已完成</span><strong>${summary.completed}</strong></div>
        <div class="stat-block"><span>已有成绩</span><strong>${summary.graded}</strong></div>
      </div>
      <div class="course-type-grid">
        ${summary.byType.length ? summary.byType.map((row) => `
          <article class="course-type-card">
            <strong>${escapeHtml(row.type)}</strong>
            <span>${row.total} 门 / 完成 ${row.completed} / 成绩 ${row.graded}${row.average === null ? "" : ` / 均分 ${row.average}`}</span>
          </article>
        `).join("") : `<div class="empty-state">暂无课程类型统计。</div>`}
      </div>
    ` : `<div class="empty-state">请先选择学生。</div>`;

    list.innerHTML = summary.records.map((record) => `
      <article class="record-item">
        <div class="student-item" style="border: 0; padding: 0;">
          <div>
            <div class="item-title">${escapeHtml(record.course?.name || "未知课程")}</div>
            <div class="item-meta">
              <span class="pill teal">${escapeHtml(record.status)}</span>
              ${record.types.map((type) => `<span class="pill blue">${escapeHtml(type)}</span>`).join("")}
              <span>${escapeHtml(record.term || "未设周期")}</span>
              <span>参与：${escapeHtml(record.attendanceCount || "未填")}</span>
              ${record.grade ? `<span class="pill gold">总评 ${escapeHtml(record.grade.score)} 分</span>` : `<span>暂无成绩</span>`}
            </div>
            ${record.note ? `<p class="muted-text">${escapeHtml(record.note)}</p>` : ""}
          </div>
          ${record.source === "enrollment" ? `
            <div class="item-actions">
              <button class="delete-button" data-delete-enrollment="${record.id}" type="button">删除</button>
            </div>
          ` : ""}
        </div>
      </article>
  ...
  function renderStudents() {
    ...
    state.students
      ...
      .forEach((student) => {
        ...
        const courseSummary = studentCourseSummary(student.id);
        ...
        item.innerHTML = `
          <div>
            ...
            <div class="item-meta">
              ...
              <span>修课：${courseSummary.total} 门</span>
  ...
  function studentFieldValue(student, key) {
    ...
    const values = {
      ...
      courseTotal: courseSummary.total,
      courseTypes: courseSummary.byType.map((row) => `${row.type}:${row.total}`).join("；")
    };
    return values[key] ?? "";
  }
  ...
  function motivationItem(record) {
    const student = getStudent(record.studentId);
    const reportVisible = motivationReportVisible(record);
    ...
    wrapper.innerHTML = `
      ...
      <div class="item-meta">
        ...
        <span class="pill ${record.statusTone === "需要支持" ? "gold" : record.statusTone === "明显波动" ? "red" : ""}">${escapeHtml(record.statusTone)}</span>
            <span class="pill ${reportVisible ? "blue" : "red"}">${reportVisible ? "报告素材" : "仅内部"}</span>
      ...
      <p class="muted-text">行动建议：${escapeHtml(record.suggestion || "暂无")}</p>
        ...
        <div class="item-actions">
          <button class="secondary-button small-button" data-toggle-motivation-report="${record.id}" type="button">${reportVisible ? "设为内部" : "加入报告"}</button>
        </div>
      </div>
    `;
    return wrapper;
  ...
  function courseOccurrenceLabel(course, start, end) {
    return `${course.name} · ${formatDateTime(localDateTimeInputValue(start))}${end ? ` - ${formatDateTime(localDateTimeInputValue(end))}` : ""}`;
  }

  function courseCalendarEventsForMonth(monthDate) {
    const monthStart = new Date(monthDate.getFullYear(), monthDate.getMonth(), 1);
    const monthEnd = new Date(monthDate.getFullYear(), monthDate.getMonth() + 1, 0, 23, 59, 59);
    return state.courses
      .filter((course) => course.startAt)
      .flatMap((course) => courseOccurrences(course, monthStart, monthEnd).flatMap((occurrence) => {
        const days = [];
        const cursor = new Date(Math.max(startOfDay(occurrence.start), startOfDay(monthStart)));
        const last = new Date(Math.min(startOfDay(occurrence.end), startOfDay(monthEnd)));
        while (cursor <= last) {
          const sameAsStart = sameDate(cursor, occurrence.start);
          const sameAsEnd = sameDate(cursor, occurrence.end);
          const displayStart = sameAsStart
            ? occurrence.start
            : new Date(cursor.getFullYear(), cursor.getMonth(), cursor.getDate(), 0, 0);
          const displayEnd = sameAsEnd
            ? occurrence.end
            : new Date(cursor.getFullYear(), cursor.getMonth(), cursor.getDate(), 23, 59);
          days.push({
            course,
            start: displayStart,
            end: displayEnd,
            ...
            colorClass: calendarColorClass(course)
          });
          cursor.setDate(cursor.getDate() + 1);
        }
        return days;
  ...
  function renderCourseMonthCalendar(monthDate) {
    ...
    courseCalendarEventsForMonth(firstDay).forEach((eventItem) => {
      const key = dateKey(eventItem.start);
      if (!eventsByDate.has(key)) eventsByDate.set(key, []);
      eventsByDate.get(key).push(eventItem);
    });
    ...
    const cells = Array.from({ length: 42 }, (_, index) => {
      ...
      const events = (eventsByDate.get(key) || []).sort((a, b) => a.start - b.start);
      const isOtherMonth = date.getMonth() !== firstDay.getMonth();
      const isToday = sameDate(date, new Date());
      return `
        <div class="course-month-cell ${isOtherMonth ? "muted" : ""} ${isToday ? "today" : ""}">
          <div class="course-month-date">${date.getDate()}</div>
          <div class="course-day-blocks">
            ${events.map((eventItem) => `
              <button
                class="course-day-block ${eventItem.colorClass}"
                data-course-calendar-course="${eventItem.course.id}"
                data-course-calendar-start="${escapeHtml(localDateTimeInputValue(eventItem.start))}"
                data-course-calendar-end="${escapeHtml(localDateTimeInputValue(eventItem.end))}"
                title="${escapeHtml(courseOccurrenceLabel(eventItem.course, eventItem.start, eventItem.end))}"
                aria-label="${escapeHtml(courseOccurrenceLabel(eventItem.course, eventItem.start, eventItem.end))}"
                type="button"
              ></button>
            `).join("")}
          </div>
    ...
    return `
      <article class="course-month-panel">
        <div class="course-month-title">${escapeHtml(monthTitle)}</div>
        <div class="course-month-weekdays">${weekdays.map((day) => `<span>${day}</span>`).join("")}</div>
        <div class="course-month-grid">${cells}</div>
      </article>
    `;
  }

  function renderCourseQuarterCalendar() {
    const input = $("#courseCalendarMonth");
    const grid = $("#courseQuarterCalendar");
    if (!input || !grid) return;
    input.value = monthInputValue(courseCalendarCursor);
    const months = [0, 1, 2].map((offset) => new Date(courseCalendarCursor.getFullYear(), courseCalendarCursor.getMonth() + offset, 1));
    grid.innerHTML = months.map(renderCourseMonthCalendar).join("");
    renderCourseCalendarDetail();
  }

  function renderCourseCalendarDetail() {
    const detail = $("#courseCalendarDetail");
    if (!detail) return;
    if (!selectedCourseCalendarEvent) {
      detail.innerHTML = `<div class="empty-state">点击日历色块查看课程详情。</div>`;
      return;
    }
    const course = getCourse(selectedCourseCalendarEvent.courseId);
    if (!course) {
      selectedCourseCalendarEvent = null;
      detail.innerHTML = `<div class="empty-state">课程不存在或已删除。</div>`;
      return;
    }
    const paths = coursePaths(course);
    const attributes = courseAttributes(course);
    detail.innerHTML = `
      <article class="record-item selected-course-detail">
        <div>
          <div class="item-title">${escapeHtml(course.name)}</div>
          <div class="item-meta">
            <span class="pill teal">${escapeHtml(course.center)}</span>
            ${paths.map((path) => `<span class="pill gold">${escapeHtml(path)}</span>`).join("")}
            ${attributes.map((attribute) => `<span class="pill violet">${escapeHtml(attribute)}</span>`).join("")}
            <span>${escapeHtml(formatDateTime(selectedCourseCalendarEvent.startAt))} - ${escapeHtml(formatDateTime(selectedCourseCalendarEvent.endAt))}</span>
            <span>老师：${escapeHtml(course.teacher || "未设")}</span>
            ${course.repeatWeekly ? `<span class="pill blue">${escapeHtml(recurrenceText(course))}</span>` : ""}
          </div>
          <p class="muted-text">${escapeHtml(course.description || "暂无说明")}</p>
        </div>
        <div class="item-actions">
          <button class="primary-button small-button" data-edit-course-occurrence="${course.id}" type="button">编辑这个时间</button>
        </div>
      </article>
    `;
  }
  ...
  function generateReport(studentId) {
    ...
    const range = reportDateRange();
    const selectedMotivationIds = new Set($all('#reportMotivationPicker input[name="reportMotivationIds"]:checked').map((input) => input.value));
    const motivation = state.motivation
      .filter((record) => record.studentId === studentId)
      .filter((record) => dateInRange(record.createdAt, range))
      .filter((record) => selectedMotivationIds.has(record.id));
    const grades = state.grades
      .filter((grade) => grade.studentId === studentId)
      .filter((grade) => dateInRange(grade.createdAt, range));
    const attendance = state.attendance
      .filter((record) => record.studentId === studentId)
      .filter((record) => dateInRange(record.date, range));
    const academy = getAcademy(student.academyId);
    const gpa = calculateGpa(studentId);
    const periodGpa = calculateGpaFromGrades(grades.filter((grade) => grade.countsGpa));
    const creditTotal = calculateCredit(studentId);
    const courseSummary = studentCourseSummary(studentId, range);
    const creditText = attendance.length ? `当前 ${creditTotal} 分，本期 ${attendance.length} 条加减记录` : `当前 ${creditTotal} 分，本期暂无加减记录`;

    $("#reportOutput").innerHTML = `
      <div class="report-section">
        <h2>${escapeHtml(student.name)} 成长档案草稿</h2>
        <p class="muted-text">学号：${escapeHtml(student.studentNo)} · 项目：${escapeHtml(student.program)} · 状态：${escapeHtml(normalizeStudentStatus(student.status))} · 报告周期：${escapeHtml(reportRangeText(range))}</p>
      </div>
      <div class="report-section">
        <h3>基础概览</h3>
        <ul>
          ...
          <li>修课记录：${courseSummary.total} 门，已完成 ${courseSummary.completed} 门，已有成绩 ${courseSummary.graded} 门</li>
          <li>GPA：${periodGpa.gpa ?? "暂无"}，本期计入课程 ${periodGpa.count} 门${range.start || range.end ? `；累计 GPA ${gpa.gpa ?? "暂无"}` : ""}</li>
          <li>信用积分：${escapeHtml(creditText)}</li>
        </ul>
      </div>
      <div class="report-section">
        <h3>课程修读统计</h3>
        ${courseSummary.byType.length ? `<ul>${courseSummary.byType.map((row) => `
          <li>${escapeHtml(row.type)}：${row.total} 门，完成 ${row.completed} 门，已有成绩 ${row.graded} 门${row.average === null ? "" : `，平均分 ${escapeHtml(row.average)}`}</li>
        `).join("")}</ul>` : `<p class="muted-text">暂无修课记录。</p>`}
        ${courseSummary.records.length ? `<ul>${courseSummary.records.map((record) => `
          <li>${escapeHtml(record.course?.name || "未知课程")}（${escapeHtml(record.types.join(" / "))}）：${escapeHtml(record.status)}，${escapeHtml(record.term || "未设周期")}${record.grade ? `，总评 ${escapeHtml(record.grade.score)} 分` : "，暂无成绩"}</li>
        `).join("")}</ul>` : ""}
      </div>
      <div class="report-section">
        <h3>动力中心记录</h3>
  ...
  function setCourseEditMode(courseId, occurrence = null) {
    ...
    form.elements.startAt.value = occurrence?.startAt || course.startAt || "";
    form.elements.endAt.value = occurrence?.endAt || course.endAt || "";
    ...
    title.textContent = occurrence ? `编辑课程时间：${course.name}` : `编辑课程：${course.name}`;
    submit.textContent = "保存修改";
    cancel.hidden = false;
  }
  ...
  function setupEvents() {
    ...
    $("#courseEnrollmentStudent").addEventListener("change", () => {
      renderCourseEnrollmentStats();
    });

    $("#courseCalendarMonth").addEventListener("input", (event) => {
      const [year, month] = event.target.value.split("-").map(Number);
      if (!year || !month) return;
      courseCalendarCursor = new Date(year, month - 1, 1);
      selectedCourseCalendarEvent = null;
      renderCourseQuarterCalendar();
    });
    ...
    $("#motivationForm").addEventListener("submit", (event) => {
      ...
      state.motivation.push({
        ...
        reportVisible: Boolean(form.elements.reportVisible.checked),
        createdAt: new Date().toISOString()
      });
      saveState();
      resetForm(form);
      ...
      if (editingCourseId) {
        ...
        if (payload.startAt) {
          ...
          if (!Number.isNaN(start.getTime())) {
            courseCalendarCursor = new Date(start.getFullYear(), start.getMonth(), 1);
            selectedCourseCalendarEvent = {
    ...
    $("#courseEnrollmentForm").addEventListener("submit", (event) => {
      event.preventDefault();
      const form = event.currentTarget;
      const data = formData(form);
      const term = data.term.trim();
      if (!data.studentId || !data.courseId) return;
      const existing = state.enrollments.find((record) => {
      ...
      const payload = {
        ...
        term,
        status: enrollmentStatuses.includes(data.status) ? data.status : "进行中",
      ...
      if (existing) {
        ...
        state.enrollments.push({ id: id("enroll"), ...payload });
      }
      saveState();
      resetForm(form);
      renderAll();
    ...
    $("#gradeForm").addEventListener("submit", (event) => {
      ...
      upsertEnrollmentFromGrade(data.studentId, data.courseId, data.term);
      saveState();
      resetForm(form);
    ...
    document.body.addEventListener("click", (event) => {
      ...
      if (event.target.closest("[data-course-calendar-prev]")) {
        courseCalendarCursor = new Date(courseCalendarCursor.getFullYear(), courseCalendarCursor.getMonth() - 3, 1);
        selectedCourseCalendarEvent = null;
        renderCourseQuarterCalendar();
        return;
      }

      if (event.target.closest("[data-course-calendar-next]")) {
        courseCalendarCursor = new Date(courseCalendarCursor.getFullYear(), courseCalendarCursor.getMonth() + 3, 1);
        selectedCourseCalendarEvent = null;
        renderCourseQuarterCalendar();
        return;
      ...
      if (event.target.closest("[data-course-calendar-today]")) {
        courseCalendarCursor = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
        selectedCourseCalendarEvent = null;
        renderCourseQuarterCalendar();
        return;
      }

      const courseCalendarBlock = event.target.closest("[data-course-calendar-course]");
      if (courseCalendarBlock) {
        selectedCourseCalendarEvent = {
          courseId: courseCalendarBlock.dataset.courseCalendarCourse,
          startAt: courseCalendarBlock.dataset.courseCalendarStart,
          endAt: courseCalendarBlock.dataset.courseCalendarEnd
        };
        renderCourseCalendarDetail();
        return;
      }
      ...
      const editCourseOccurrenceButton = event.target.closest("[data-edit-course-occurrence]");
      if (editCourseOccurrenceButton) {
        setCourseEditMode(editCourseOccurrenceButton.dataset.editCourseOccurrence, selectedCourseCalendarEvent);
        $("#courseForm")?.scrollIntoView({ behavior: "smooth", block: "start" });
        return;
      }

      const toggleMotivationReportButton = event.target.closest("[data-toggle-motivation-report]");
      if (toggleMotivationReportButton) {
        const record = state.motivation.find((item) => item.id === toggleMotivationReportButton.dataset.toggleMotivationReport);
        if (!record) return;
        record.reportVisible = !motivationReportVisible(record);
        saveState();
        renderAll();
        return;
      }
      ...
      const enrollmentButton = event.target.closest("[data-delete-enrollment]");
      if (enrollmentButton) {
        const enrollmentId = enrollmentButton.dataset.deleteEnrollment;
        ...
        if (!window.confirm(`确定删除“${student?.name || "该学生"}”的“${course?.name || "该课程"}”修课记录吗？成绩记录不会被删除。`)) return;
        state.enrollments = state.enrollments.filter((item) => item.id !== enrollmentId);
        saveState();
        renderAll();
        return;
      }
